...
source('/home/z***/script/v8/utmost/glasso.r')

#get full gene list
gene_list_geno<-dir('/data/c***/z***/data/gtex/geno/v8/gene/p_1m/')
gene_list_exp<-sub('....$','',dir('/data/c***/z***/projects/cross_tissue/exp_gene/'))
...
for (gene_i in i_start:i_end){
  ...
  ###input data prepare###
  #format transfer for expression data
  dir.create(paste0('/data/c***/z***/projects/cross_tissue/utmost/exp/',gene_id))
  exp<-readRDS(paste0('/data/c***/z***/projects/cross_tissue/exp_gene/',gene_id,'.rds'))
  tissue_list<-unique(exp$tissue)
  for (tissue in tissue_list){
    exp_tmp<-exp[exp$tissue==tissue,-3]
    saveRDS(exp_tmp,paste0('/data/c***/z***/projects/cross_tissue/utmost/exp/',gene_id,'/',tissue,'.rds'))
  }
  
  #rename sampleid of genotype dosage files
  geno<-read.table(paste0('/data/c***/z***/data/gtex/geno/v8/gene/p_1m/',gene_id),header = T,stringsAsFactors = F)
  geno[,1]<-paste0('GTEX.',geno[,1])
  saveRDS(geno,paste0('/data/c***/z***/projects/cross_tissue/utmost/geno/',gene_id,'.rds'))
  
  #generate genotype info files
  geno_info<-read.table(paste0('/data/c***/z***/data/gtex/geno/v8/gene/p_1m_info/',gene_id),header = T,stringsAsFactors = F)
  chr_pos<-geno_info[,c(1,2)]
  geno_info<-geno_info[,-2]
  colnames(geno_info)<-c('SNP','REF.0.','ALT.1.')
  write.table(geno_info,paste0('/data/c***/z***/projects/cross_tissue/utmost/info/',gene_id),quote = F,row.names = F,sep='\t')
  
  ### data import ###
  main_dir='/data/c***/z***/projects/cross_tissue/utmost/'
  ...
  if(P){
    ...
    for (tissue_i in 1:length(Yt)){
      ...
      if(nrow(weight_df)>0 & cv_r[tissue_i]>0.1 & cv_p[tissue_i]<0.05){
        print(tissue_i)
        saveRDS(weight_df,paste0('/data/c***/z***/projects/v8/weights_cv/ut/',tissue_name,'/',gene_id,'.rds'))
      }
